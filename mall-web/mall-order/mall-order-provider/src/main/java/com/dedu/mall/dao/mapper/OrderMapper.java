...
//    void save(OrderPo orderPo);
    ...
    @Select("SELECT o.id AS id, o.buyer_id AS buyerId, o.total_price AS totalPrice, o.title AS title, o.create_time AS createTime, o.update_time AS updateTime, o.status AS status, p.total_pay AS totalPay, p.pay_type AS payType FROM tb_order o LEFT JOIN tb_pay p ON o.id = p.order_id WHERE o.buyer_id = #{userId} LIMIT #{pageNum}, #{pageSize}")
    List<OrderAllInfoPo> getOrderAllInfoPageByUserId(@Param(value = "userId") Long userId, @Param(value = "pageNum") Integer pageNum, @Param(value = "pageSize") Integer pageSize);

    @Select("SELECT COUNT(*) FROM tb_order o LEFT JOIN tb_pay p ON o.id = p.order_id WHERE o.buyer_id = #{userId}")
    Integer getOrderAllInfoCountByUserId(@Param(value = "userId") Long userId);
    ...
    @Select("SELECT s.STATUS FROM tb_order_status s WHERE order_id = #{orderId}")
    Integer getOrderStatusByOrderId(String id);
}
